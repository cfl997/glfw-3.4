...
//cfl
static DWORD getWindowStyleParent(const _GLFWwindow* window)
{
    DWORD style = WS_CLIPSIBLINGS | WS_CLIPCHILDREN;
...
//
static int createNativeWindow(void* parent, _GLFWwindow* window,
                              ...
                              const _GLFWfbconfig* fbconfig)
{
    ...
    DWORD style;
    if (parent)
        style = getWindowStyleParent(window);
    else
        style = getWindowStyle(window);
...
GLFWbool _glfwCreateWindowWin32(void*parent, _GLFWwindow* window,
                                const _GLFWwndconfig* wndconfig,
                                const _GLFWctxconfig* ctxconfig,
                                const _GLFWfbconfig* fbconfig)
{
    if (!createNativeWindow(parent, window, wndconfig, fbconfig))
        return GLFW_FALSE;
